...
## Supabase Setup (Deferred Until Production Integration)

The active development build currently uses local browser data and a local
development login. Keep this setup for the later production database phase.
...
# Apply security hardening to privileged billing functions
supabase db push < supabase/migrations/002_security_hardening.sql
...
- Use only a Supabase publishable key in `frontend/.env`; never expose a secret or `service_role` key.
- Apply `supabase/migrations/002_security_hardening.sql` to prevent browser-supplied sale pricing and cashier impersonation.
- In Supabase Authentication settings, enable CAPTCHA/bot protection and review Auth rate limits before public launch.
- Enable MFA for admin users and require strong unique passwords.
- Run Supabase Security Advisor after schema or policy changes and address findings before deployment.
- Rotate any database password or secret that has been shared in messages, logs, or screenshots.
